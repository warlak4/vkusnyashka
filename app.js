...
function renderNav() {
  ...
  return `
    <nav style="margin-bottom:2rem;display:flex;align-items:center;gap:1.5rem;flex-wrap:wrap;">
      <span>
        <a onclick="renderHome()">Главная</a> |
        <a onclick="renderCatalog()">Каталог</a> |
        <a onclick="renderCreateCake()">Создай свой торт</a> |
        <a onclick="renderSets()">Наборы со скидкой</a> |
        <a onclick="renderCart()">Корзина</a> |
        <a onclick="renderAbout()">О компании</a> |
        <a onclick="renderContacts()">Контакты</a>
        ${user ? `| <a onclick="renderOrderHistory()">История заказов</a>` : ''}
        ${user && user.role === 'admin' ? '| <a onclick="renderAdminPanel()">Панель админа</a>' : ''}
      </span>
      <span style="margin-left:auto;display:flex;align-items:center;gap:0.7rem;">
        ${user ? 
          `<span class="user-badge">${user.name} (${user.role === 'admin' ? 'админ' : 'пользователь'})</span> <button class="nav-btn" onclick="logout()">Выйти</button>` :
